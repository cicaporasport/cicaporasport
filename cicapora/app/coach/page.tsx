...
export default function CoachPage() {
  ...
  // Fetch jadwal setiap kali selectedAtlet berubah
  useEffect(() => {
    if (selectedAtlet) {
  ...
  const saveSession = (e: React.FormEvent) => {
    ...
    const newSession: TrainingSession = { 
      ...formSession, 
      id: Date.now(), 
      NamaAtlet: selectedAtlet 
    };

    const updatedSessions = [...sessions, newSession];
    setSessions(updatedSessions);
    localStorage.setItem('coachSessions', JSON.stringify(updatedSessions));
  ...
  // ==================== SIMPAN PRESTASI ====================
  const savePrestasi = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const newPrestasi: PrestasiAtlet = { id: Date.now(), ...formPrestasi };
    const updated = [...prestasiList, newPrestasi];
    setPrestasiList(updated);
    localStorage.setItem('atletPrestasi', JSON.stringify(updated));
  ...
  // ==================== SIMPAN PROGRES PANJAT TEBING ====================
  const saveClimbingProgress = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedAtlet) return alert('Pilih Atlet terlebih dahulu!');

    const newWeek: ClimbingTrainingWeek = {
    ...
    const current = climbingData[selectedAtlet] || [];
    let updatedAthleteData = [...current, newWeek];

    if (updatedAthleteData.length > 12) {
      updatedAthleteData = updatedAthleteData.slice(-12);
    }

    const newClimbingData = {
      ...climbingData,
      [selectedAtlet]: updatedAthleteData
    };

    setClimbingData(newClimbingData);
    localStorage.setItem('climbingProgress', JSON.stringify(newClimbingData));
  ...
  const deleteClimbingWeek = (athleteName: string, minggu: number) => {
    if (!confirm(`Yakin ingin menghapus data Minggu ke-${minggu} untuk ${athleteName}?`)) return;

    const current = climbingData[athleteName] || [];
    const updated = current.filter(w => w.minggu !== minggu);
